...
# 2 lanes for each sample, merge into 1 file:
desc "merge files"
task :merge_files, :roles => group_name do
  run "cat #{mount_point}/SCS785*.txt > #{mount_point}/input_export.txt"
  run "cat #{mount_point}/CME026*.txt > #{mount_point}/chip_export.txt"
  run "rm #{mount_point}/SCS785*"
  run "rm #{mount_point}/CME026*"
end
before "merge_files", "EC2:start"
